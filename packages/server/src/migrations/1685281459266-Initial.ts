...
export class Initial1685281459266 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE TABLE \`permission\` (\`name\` enum ('GRANT:PERMISSION', 'MANAGE:USER', 'UPLOAD:IMAGE', 'UPLOAD:VIDEO', 'MANAGE:SERIES', 'FETCH:SERIES', 'MANAGE:SUBSCRIBE', 'FETCH:SUBSCRIBE', 'MANAGE:ARIA2') NOT NULL, \`description\` varchar(255) NULL, PRIMARY KEY (\`name\`)) ENGINE=InnoDB`);
    ...
    await queryRunner.query(`CREATE TABLE \`user_permissions_permission\` (\`userId\` int NOT NULL, \`permissionName\` enum ('GRANT:PERMISSION', 'MANAGE:USER', 'UPLOAD:IMAGE', 'UPLOAD:VIDEO', 'MANAGE:SERIES', 'FETCH:SERIES', 'MANAGE:SUBSCRIBE', 'FETCH:SUBSCRIBE', 'MANAGE:ARIA2') NOT NULL, INDEX \`IDX_5b72d197d92b8bafbe7906782e\` (\`userId\`), INDEX \`IDX_7691aabb0be21b97826f754a31\` (\`permissionName\`), PRIMARY KEY (\`userId\`, \`permissionName\`)) ENGINE=InnoDB`);
