...
export class Schema1705211396104 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`permission\` (\`name\` enum ('*:*', 'FILE:*', 'FILE:UPLOAD:IMAGE', 'FILE:UPLOAD:VIDEO', 'MEDIA:*', 'MEDIA:VIEW', 'SUBSCRIBE:*', 'SUBSCRIBE:VIEW', 'LIVE:*', 'LIVE:VIEW') NOT NULL, \`userId\` int NOT NULL, PRIMARY KEY (\`name\`, \`userId\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`notification_subscribe\` (\`name\` enum ('new-episode', 'new-media') NOT NULL, \`metaId\` int NOT NULL, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`name\`, \`metaId\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`notification_meta\` (\`id\` int NOT NULL AUTO_INCREMENT, \`service\` enum ('EMAIL', 'WS') NOT NULL, \`enabled\` tinyint NOT NULL DEFAULT 0, \`config\` text NULL, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`userId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`action_log\` (\`id\` varchar(36) NOT NULL, \`ip\` varchar(255) NOT NULL, \`action\` enum ('LOGIN', 'LOGOUT', 'REFRESH', 'GRANT', 'CHANGE_PASSWORD') NOT NULL, \`extra\` text NULL, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`operatorId\` int NULL, \`targetId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`notification_subscribe\` ADD CONSTRAINT \`FK_bc4087e212c1de95ebe644ce2d4\` FOREIGN KEY (\`metaId\`) REFERENCES \`notification_meta\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`notification_subscribe\` DROP FOREIGN KEY \`FK_bc4087e212c1de95ebe644ce2d4\``);
        ...
        await queryRunner.query(`DROP TABLE \`notification_subscribe\``);
